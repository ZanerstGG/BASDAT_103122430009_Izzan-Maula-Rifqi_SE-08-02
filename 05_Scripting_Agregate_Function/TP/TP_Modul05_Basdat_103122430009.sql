...
GROUP BY m.NAMA, m.TGL_LAHIR;

select
    TO_CHAR(TGL_LAHIR, 'Month') AS NAMA_BULAN,
    EXTRACT(MONTH FROM TGL_LAHIR) AS ANGKA_BULAN,
    COUNT(*) AS JUMLAH_MEMBER
from MEMBER
GROUP BY TO_CHAR(TGL_LAHIR, 'Month'), EXTRACT(MONTH FROM TGL_LAHIR)
ORDER BY ANGKA_BULAN;
