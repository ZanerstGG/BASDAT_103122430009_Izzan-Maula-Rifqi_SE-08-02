...
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE "INVENTORI" CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN NULL; END;
/
...
SELECT 
    ...
    TO_CHAR(j.PERIODE_START, 'MM-DD-YYYY') AS PERIODE_START, 
    EXTRACT(DAY FROM j.PERIODE_END) || ' hari' AS "LAMA WAKTU"
FROM FILM f
...
SELECT 
    ...
    CASE 
        WHEN (EXTRACT(YEAR FROM TO_DATE('2021-01-01', 'YYYY-MM-DD')) - EXTRACT(YEAR FROM m.TGL_LAHIR)) <= 32 THEN t.HARGA * 0.9 
        ELSE t.HARGA 
    END AS SALE
FROM TRANSAKSI tr
...
SELECT 
    ...
    t.HARGA, 
    CASE 
        WHEN f.JUDUL = 'Keluarga Cemara' THEN t.HARGA * 0.7 
        ELSE t.HARGA 
    END AS "Sale"
FROM TRANSAKSI tr
JOIN JADWAL j ON tr.ID_JADWAL = j.ID_JADWAL
JOIN FILM f ON j.ID_FILM = f.ID_FILM
JOIN THEATER t ON j.ID_THEATER = t.ID_THEATER
WHERE tr.TANGGAL >= TO_DATE('2019-06-19', 'YYYY-MM-DD');